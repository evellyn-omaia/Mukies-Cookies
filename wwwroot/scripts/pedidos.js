...
function configurarEventosPedidos() {
  ...
  document.querySelectorAll(".resumo-pedido[data-status]").forEach(function (card) {
    card.addEventListener("click", function () {
      const status = Number(card.dataset.status);
      estadoPedidos.statusCard = estadoPedidos.statusCard === status ? 0 : status;
  ...
  elementosPedidos.adicionarItem.addEventListener("click", adicionarLinhaItem);
...
async function carregarProdutosDisponiveis() {
  try {
    const produtos = await requisicaoApi("/produtos?disponivel=true");
...
function filtrarPedidos() {
  ...
  const status = estadoPedidos.statusCard || Number(elementosPedidos.filtroStatus.value);
...
function renderizarTabela() {
    ...
    .map(function (pedido) {
      ...
      return `
        <tr class="linha-pedido${selecionado ? " selecionado" : ""}"
            data-pedido-id="${pedido.id}" tabindex="0" aria-selected="${selecionado}">
          <td><strong class="numero-pedido">#${pedido.id}</strong></td>
          <td><span class="cliente-pedido" title="${escaparHtml(pedido.nomeCliente)}">${escaparHtml(pedido.nomeCliente)}</span></td>
          <td>${formatarDataPedido(pedido.dataPedido)}</td>
          <td><span class="status-pedido ${status.classe}">${status.nome}</span></td>
          <td><strong>${formatarMoeda(pedido.valorTotal)}</strong></td>
          <td>
            <div class="acoes-pedido">
              <button type="button" class="acao-pedido" aria-label="Ver detalhes do pedido #${pedido.id}">
  ...
  elementosPedidos.corpoTabela.querySelectorAll(".linha-pedido").forEach(function (linha) {
...
function renderizarPaginacao() {
  ...
  const inicio = quantidade ? (estadoPedidos.pagina - 1) * PEDIDOS_POR_PAGINA + 1 : 0;
  ...
  let html = criarBotaoPagina("‹", estadoPedidos.pagina - 1, estadoPedidos.pagina === 1);

  paginas.forEach(function (pagina) {
    if (pagina === "...") {
      html += '<span class="paginacao-reticencias">…</span>';
    } else {
      html += criarBotaoPagina(String(pagina), pagina, false, pagina === estadoPedidos.pagina);
    }
  });
  ...
  elementosPedidos.paginacao.querySelectorAll("button[data-pagina]").forEach(function (botao) {
...
function criarBotaoPagina(texto, pagina, desabilitado, ativo) {
  return `<button type="button" class="pagina-btn${ativo ? " ativa" : ""}"
    data-pagina="${pagina}" ${desabilitado ? "disabled" : ""}>${texto}</button>`;
}
...
function renderizarDetalhes(pedido) {
  ...
  elementosPedidos.detalhes.innerHTML = `
    ...
    <div class="dados-pedido">
      <div class="dado-pedido"><i data-lucide="user"></i><span>${escaparHtml(pedido.nomeCliente)}</span></div>
      ${pedido.telefoneCliente ? `<div class="dado-pedido"><i data-lucide="phone"></i><a href="tel:${escaparHtml(pedido.telefoneCliente)}">${escaparHtml(pedido.telefoneCliente)}</a></div>` : ""}
      <div class="dado-pedido"><i data-lucide="calendar-days"></i><span>${formatarDataPedido(pedido.dataPedido, true)}</span></div>
      <div class="dado-pedido"><i data-lucide="message-square-text"></i><span>${observacao}</span></div>
    </div>
    <h4 class="titulo-itens-detalhe">Itens do pedido (${pedido.quantidadeTotalItens})</h4>
    ...
    <div class="detalhe-acoes">
      ...
      <select id="statusDetalhesPedido">${criarOpcoesStatus(pedido.status)}</select>
      <button type="button" class="botao-detalhe" id="excluirPedidoSelecionado">
        <i data-lucide="trash-2"></i> Excluir pedido
      </button>
    </div>`;

  document.getElementById("statusDetalhesPedido").addEventListener("change", function (evento) {
    alterarStatusPedido(pedido.id, Number(evento.target.value), evento.target);
  });

  document.getElementById("excluirPedidoSelecionado").addEventListener("click", function () {
...
function atualizarResumos() {
  ...
  const totalHoje = estadoPedidos.pedidos
    .filter(function (pedido) {
      return pedido.status !== 6 && pedidoDentroDoPeriodo(pedido.dataPedido, "hoje");
    })
...
function marcarCardSelecionado() {
  document.querySelectorAll(".resumo-pedido[data-status]").forEach(function (card) {
    card.classList.toggle("ativo", Number(card.dataset.status) === estadoPedidos.statusCard);
  });
}
...
function adicionarLinhaItem() {
  ...
  const opcoes = estadoPedidos.produtos
    .map(function (produto) {
      return `<option value="${produto.id}">${escaparHtml(produto.nome)} — ${formatarMoeda(produto.preco)}</option>`;
    })
    .join("");
  ...
  linha.querySelector("select").addEventListener("change", atualizarFormularioItens);
  linha.querySelector("input").addEventListener("input", atualizarTotalNovoPedido);
...
function atualizarFormularioItens() {
  const seletores = elementosPedidos.itens.querySelectorAll(".produto-item-pedido");
  ...
  seletores.forEach(function (seletor) {
    Array.from(seletor.options).forEach(function (opcao) {
      if (opcao.value) {
        opcao.disabled = opcao.value !== seletor.value && produtosEscolhidos.includes(opcao.value);
      }
    });
  });

  elementosPedidos.adicionarItem.disabled = seletores.length >= estadoPedidos.produtos.length;
...
function atualizarTotalNovoPedido() {
  ...
  elementosPedidos.itens.querySelectorAll(".item-pedido-linha").forEach(function (linha) {
...
async function salvarNovoPedido(evento) {
  ...
  try {
    ...
    const pedidoCriado = await requisicaoApi("/pedidos", {
      method: "POST",
    ...
    fecharModalPedido();
    limparFiltros();
    estadoPedidos.pedidoSelecionado = pedidoCriado.id;
    await carregarPedidos();
  } catch (erro) {
    mostrarErroFormulario(erro.message);
  } finally {
    elementosPedidos.salvar.disabled = false;
    elementosPedidos.salvar.innerHTML = '<i data-lucide="check"></i>Salvar pedido';
    atualizarIconesPedidos();
  }
}
